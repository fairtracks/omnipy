...
def test_job_creator_singular_mock(
    ...
    with pytest.raises(AttributeError):
        JobBase.job_creator = JobCreator()  # type: ignore[misc]

    with pytest.raises(AttributeError):
        JobTemplate.job_creator = JobCreator()  # type: ignore[attr-defined]

    with pytest.raises(AttributeError):
        Job.job_creator = JobCreator()  # type: ignore[attr-defined]

    job_creator = JobBase.job_creator

    assert cast(type[JobBase], JobTemplate).job_creator is job_creator
    assert cast(type[JobBase], Job).job_creator is job_creator

    job_tmpl = JobTemplate()
    assert cast(JobBase, job_tmpl).__class__.job_creator is job_creator

    job = job_tmpl.apply()
    job_as_job_base = cast(JobBase, job)
    assert job_as_job_base.__class__.job_creator is job_creator

    job_tmpl_new = JobTemplate()
    assert cast(JobBase, job_tmpl_new).__class__.job_creator is job_creator
...
def test_job_creator_properties_mock(
    ...
    for test in [
        ...
        JobTemplateAsJobBase = cast(type[JobBase], JobTemplate)
        if test.enter_exit:
            JobTemplateAsJobBase.job_creator.__enter__()
        else:
            assert test.set_method
            getattr(JobTemplateAsJobBase.job_creator, test.set_method)(test.val)
        ...
        if test.enter_exit:
            JobTemplateAsJobBase.job_creator.__exit__(None, None, None)
        else:
            assert test.set_method
...
def _assert_prop_getattr_job_subcls(job_cls: type,
                                    job_obj: IsMockJobTemplate | IsMockJob | None,
                                    property: str,
                                    in_job_obj: bool,
                                    at_obj_level: bool,
                                    val: object):
...
def _assert_immutable_command_mock_job_properties(
    ...
    with pytest.raises(AttributeError):
        cmd_obj.params = {}  # type: ignore[misc, assignment]

    with pytest.raises(TypeError):
        cmd_obj.params['what'] = 'none'  # type: ignore[index]

    with pytest.raises(TypeError):
        del cmd_obj.params['what']  # type: ignore[attr-defined]
...
def test_subclass_apply_revise() -> None:
    ...
    assert_func_wrapper(cmd_tmpl_revised, cmd)
    ...
    assert_func_wrapper(cmd_revised, cmd_tmpl_revised)
...
def test_subclass_refine_empty() -> None:
    ...
    assert_func_wrapper(cmd_tmpl_refined, cmd_tmpl)
...
def test_subclass_refine_scalar() -> None:
    ...
    assert_func_wrapper(all_erased, all_erased_tmpl)
    ...
    assert_func_wrapper(shout_tmpl, all_erased_tmpl)
    assert shout_tmpl != all_erased_tmpl
    for shout_obj in shout_tmpl, shout_tmpl.apply():
        assert_func_wrapper(shout_obj, shout_tmpl)
    ...
    assert_func_wrapper(shout, shout_tmpl)
    ...
    assert_func_wrapper(silent_tmpl, shout_tmpl)
    assert silent_tmpl != shout_tmpl
    for silent_obj in silent_tmpl, silent_tmpl.apply():
        assert_func_wrapper(silent_obj, silent_tmpl)
    ...
    assert_func_wrapper(silent, silent_tmpl)
    assert silent != shout
...
def test_subclass_refine_mapping() -> None:
    ...
    assert_func_wrapper(nicer_tmpl, cmd_tmpl)
    ...
    assert_func_wrapper(nicer, nicer_tmpl)
    assert nicer != cmd
    ...
    assert_func_wrapper(secret_tmpl, nicer_tmpl)
    assert secret_tmpl != nicer_tmpl
    for secret_obj in secret_tmpl, secret_tmpl.apply():
        assert_func_wrapper(secret_obj, secret_tmpl)
    ...
    assert_func_wrapper(secret, secret_tmpl)
    assert secret != nicer
    ...
    assert_func_wrapper(nothing_tmpl, secret_tmpl)
    assert nothing_tmpl != secret_tmpl
    for nothing_obj in nothing_tmpl, nothing_tmpl.apply():
        assert_func_wrapper(nothing_obj, nothing_tmpl)
    ...
    assert_func_wrapper(nothing, nothing_tmpl)
...
def test_subclass_refine_reset_mapping() -> None:
    ...
    assert_func_wrapper(nothing_tmpl, cmd_tmpl)
    ...
    assert_func_wrapper(reset_params_tmpl, nothing_tmpl)
...
def test_revise_refine_mappings_are_copied() -> None:
    ...
    assert_func_wrapper(all_job, all_tmpl)
    all_revised_tmpl = all_job.revise()
    assert_func_wrapper(all_revised_tmpl, all_job)
    all_revised_refined_tmpl = all_revised_tmpl.refine(params=dict(where='sadly'),)
    assert_func_wrapper(all_revised_refined_tmpl, all_revised_tmpl)
...
def test_fail_subclass_refine_first_arg_not_callable() -> None:
    ...
    with pytest.raises(TypeError):
        cmd_tmpl.refine('restore')  # type: ignore[arg-type]
