...
def test_json_dataset_array_of_objects_same_keys():
    ...
    json_data['obj'] = '[{"a": "abc", "b": 12}, {"a": "bcd", "b": 23}]'
    assert json_data['obj'] == [{'a': 'abc', 'b': 12}, {'a': 'bcd', 'b': 23}]


def test_json_dataset_array_of_objects_different_keys():
    json_data = JsonDataset()
    json_data['obj'] = '[{"a": "abc", "b": 12}, {"c": "bcd"}]'
    assert json_data['obj'] == [{'a': 'abc', 'b': 12}, {'c': 'bcd'}]


def test_json_dataset_array_of_nested_objects():
    json_data = JsonDataset()
    json_data['obj'] = '[{"a": "abc", "b": {"c": [1, 3]}}]'
    assert json_data['obj'] == [{'a': 'abc', 'b': {'c': [1, 3]}}]
...
def test_json_dataset_error_not_array_of_objects():
    json_data = JsonDataset()
    with pytest.raises(ValidationError):
        json_data['obj'] = '[123, 234]'
