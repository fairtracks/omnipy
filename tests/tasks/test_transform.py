...
def test_convert_to_pandas():
    ...
    json_data['obj_type_2'] = '[{"c": 12, "d": "aaa", "e": "xyz"}, {"c": 23, "d": "bbb", "e": "zyx"}]'

    pandas_data = convert_json_to_pandas(json_data)

    assert isinstance(pandas_data, PandasDataset)
    assert pandas_data['obj_type_1'].equals(pd.DataFrame({'a': ['abc', 'bcd'], 'b': [12, 34]}))
    assert pandas_data['obj_type_2'].equals(pd.DataFrame({'c': [12, 23], 'd': ['aaa', 'bbb'], 'e': ['xyz', 'zyx']}))
