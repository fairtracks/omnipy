...
def detect_and_setup_user_interface(runtime: 'Runtime') -> None:
    ...
    match ui_type:
        case _ui_type if UserInterfaceType.is_jupyter(_ui_type):
            ...
            display_jupyter_setup_note(ui_type)
...
def display_jupyter_setup_note(detected_ui_type: JupyterUserInterfaceType.Literals):
    ...
    if UserInterfaceType.is_jupyter_embedded(detected_ui_type):
        ...
        #     _note_mime_bundle(
        #         bullet='❗️',
        #         html_content=('If you see this message outside of a '
        #                       'web browser (e.g. from within PyCharm), '
        #                       'you probably want to restart the '
        #                       'kernel. '),
        #     ),
        #     raw=True,
        # )
...
def setup_jupyter_ui():
    ...
    display(
        note_mime_bundle(
            bullet='⚠️',
            html_content=dedent('''\
                This cell was not loaded properly! You probably want to 
                rerun the code cell above (<i>Click in the code cell, and 
                press Shift+Enter <kbd>⇧</kbd>+<kbd>↩</kbd></i>).'''),
            noscript=True,
        ),
        raw=True,
    )
