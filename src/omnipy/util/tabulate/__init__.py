...
def _asciidoc_row(is_header, *args):
    ...
    if len(args) == 2:
        ...
        raise ValueError(" _asciidoc_row() requires two (colwidths, colaligns) "
...
def _isnumber(string):
    ...
    if not _isconvertible(float, string):
        return False
                    ...
                    (str, bytes)) and (math.isinf(float(string)) or math.isnan(float(string))):
...
def _isbool(string):
    ...
    False
    """
    return type(string) is bool or (isinstance(string,
                                               (bytes, str)) and string in ("True", "False"))
...
def _append_multiline_row(lines,
